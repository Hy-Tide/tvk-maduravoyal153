export const translations = {
  ta: {
    brandName: "தமிழக வெற்றி கழகம்",
    brandSub: "மதுரவாயல் தொகுதி",

    // Navigation
    navHome: "முகப்பு",
    navAbout: "எங்களை பற்றி",
    navContact: "தொடர்புக்கு",
    navComplaint: "புகார் பதிவு",

    topBarText: "மக்களின் குரல் — TVK மதுரவாயல் தொகுதி",
    hotlineLabel: "இலவச உதவி எண்:",

    // Home Page
    heroTagline1: "மக்களுக்காக, மக்களால்",
    heroTitlePrefix: "தமிழக வெற்றி கழகம்",
    heroTitleSub: "மதுரவாயல் தொகுதி",
    heroSubtitle: "மக்களின் குரலாக, சேவை மனப்பான்மையோடு செயல்படும் மக்கள் இயக்கம்.",
    heroCtaComplaint: "புகார் பதிவு செய்க",
    heroCtaAbout: "எங்களைப் பற்றி",

    homeIntroTitle: "எங்கள் இயக்க அறிமுகம்",
    homeIntroPara: "தமிழக வெற்றி கழகம் — தமிழ் மக்களின் உரிமைகளை நிலைநாட்டவும், சமூக நீதியை உறுதிப்படுத்தவும், மதுரவாயல் தொகுதி மக்களின் குறைகளை தீர்க்கவும் அர்ப்பணிக்கப்பட்ட இயக்கம்.",

    stat1Num: "50,000+",
    stat1Label: "உறுப்பினர்கள்",
    stat2Num: "100%",
    stat2Label: "வார்டு தொடர்பு",
    stat3Num: "4,800+",
    stat3Label: "தீர்க்கப்பட்ட புகார்கள்",
    stat4Num: "24/7",
    stat4Label: "மக்கள் சேவை",

    homeQuickActionsTitle: "உடனடி சேவைகள்",
    homeAction1Title: "புகார் பதிவு மையம்",
    homeAction1Desc: "உங்கள் பகுதியில் உள்ள சாலை, குடிநீர், தெருவிளக்கு போன்ற பிரச்சினைகளை நேரடியாக பதிவு செய்யுங்கள்.",
    homeAction2Title: "தொடர்பு மற்றும் அலுவலகம்",
    homeAction2Desc: "எங்கள் தொகுதி தலைமை அலுவலக முகவரி மற்றும் தொலைபேசி எண்களை காண தொடர்புக்கு செல்லவும்.",

    // About Us Page
    aboutHeaderTitle: "எங்களை பற்றி",
    aboutHeaderSub: "",
    aboutWhoTitle: "மக்களின் குரலாக, மக்களின் தேவைகளுக்காக செயல்படும் இயக்கம்",
    aboutWhoPara1: "தமிழக வெற்றி கழகம் — தமிழ் மக்களின் உரிமைகளை நிலைநாட்டவும், சமூக நீதியை உறுதிப்படுத்தவும் தொடர்ந்து உழைக்கும் ஒரு மக்கள் இயக்கம். நமது கொள்கை: ஒவ்வொரு மனிதனும் கண்ணியமாக வாழ வேண்டும்.",
    aboutWhoPara2: "",

    aboutPurposeTitle: "",
    purpose1Title: "நம் நோக்கம்",
    purpose1Desc: "அனைவருக்கும் சம வாய்ப்பு, சம நீதி, சம உரிமை கொண்ட தமிழகம் படைத்திட.",
    purpose2Title: "நம் செயல் திட்டம்",
    purpose2Desc: "மக்கள் குறைகளை நேரடியாக கேட்டு, உரிய அதிகாரிகள் மூலம் விரைந்து தீர்வு காண்பது.",
    purpose3Title: "நம் அணுகுமுறை",
    purpose3Desc: "வெளிப்படைத்தன்மை, நேர்மை, மக்களிடையே நேரடி தொடர்பு மூலம் செயல்படுவது.",

    // Contact Page
    contactHeaderTitle: "தொடர்பு கொள்ள",
    contactHeaderSub: "எங்கள் அலுவலக முகவரி மற்றும் தொலைபேசி விவரங்கள்",
    contactOrgName: "தமிழக வெற்றி கழகம் — மதுரவாயல் தொகுதி",
    contactPhoneTitle: "தொலைபேசி எண்கள்",
    contactPhoneVal: "+91 44 2345 6789 / +91 98765 43210",
    contactTollFree: "இலவச எண்: 1800-TVK-MAD (1800 885 623)",
    contactEmailTitle: "மின்னஞ்சல் முகவரி",
    contactEmailVal: "maduravoyal@tvk.org.in",
    contactAddressTitle: "தலைமை அலுவலக முகவரி",
    contactAddressVal: "எண் 42, பிபிசி மெயின் ரோடு, மதுரவாயல், சென்னை - 600 095, தமிழ்நாடு.",
    contactHoursTitle: "வேலை நேரங்கள்",
    contactHoursVal: "திங்கள் - ஞாயிறு: காலை 9:00 - மாலை 6:00 வரை",
    contactMapTitle: "அலுவலக இருப்பிடம் (Google Maps)",
    contactMapBtn: "Google Maps இல் வழிகாட்டல் பெற",
    contactSocialTitle: "சமூக வலைதளங்களில் பின்தொடருங்கள்",

    // Complaint Page
    complaintHeaderTitle: "புகார் பதிவு மையம்",
    complaintHeaderSub: "உங்கள் புகாரை நேரடியாக பதிவு செய்யுங்கள்",
    complaintNotice: "குறிப்பு: பதிவு செய்யப்படும் அனைத்து புகார்களும் எங்கள் தொகுதி பார்வைக்கு நேரடியாக அனுப்பி வைக்கப்படும்.",

    formName: "முழு பெயர் *",
    formNamePlaceholder: "உங்கள் பெயரை உள்ளிடுக",
    formPhone: "தொலைபேசி எண் *",
    formPhonePlaceholder: "+91 98765 43210",
    formEmail: "மின்னஞ்சல் முகவரி *",
    formEmailPlaceholder: "example@domain.com",
    formAddress: "முகவரி / வார்டு விவரம் *",
    formAddressPlaceholder: "கதவு எண், தெரு பெயர், வார்டு எண்",
    formSubject: "புகாரின் தலைப்பு *",
    formSubjectPlaceholder: "எ.கா: தெருவிளக்கு பழுது / குடிநீர் விநியோகப் பிரச்சினை",
    formDetails: "புகார் விவரம் *",
    formDetailsPlaceholder: "உங்கள் பிரச்சினையை விரிவாக விளக்கமாக எழுதவும்...",
    formSubmitBtn: "புகாரை சமர்ப்பிக்குக",
    formMandatory: "* நட்சத்திரம் இட்ட புலன்கள் கட்டாயமாகும்.",

    valRequired: "அனைத்து கட்டாய புலன்களையும் நிரப்பவும்.",
    valInvalidEmail: "சரியான மின்னஞ்சல் முகவரியை உள்ளிடவும்.",
    valInvalidPhone: "சரியான 10 இலக்க தொலைபேசி எண்ணை உள்ளிடவும்.",

    successTitle: "உங்கள் புகார் வெற்றிகரமாக சமர்ப்பிக்கப்பட்டது!",
    successMessage: "உங்கள் புகார் விவரங்கள் TVK மதுரவாயல் மின்னஞ்சலுக்கு (maduravoyal@tvk.org.in) அனுப்பப்பட்டுள்ளது.",
    successTicketLabel: "புகார் குறிப்பு எண் (Ticket ID):",
    successNote: "எங்கள் கள நிர்வாகிகள் விரைவில் உங்களை தொடர்பு கொண்டு நடவடிக்கை எடுப்பார்கள்.",
    newComplaintBtn: "மற்றுமொரு புகார் பதிவு செய்ய",

    // Footer
    footerDesc: "தமிழக வெற்றி கழகம் — மதுரவாயல் தொகுதி. மக்களுக்காக, மக்களால் இயங்கும் நேர்மையான மக்கள் இயக்கம்.",
    footerQuickLinks: "பக்கங்கள்",
    footerCopyright: "தமிழக வெற்றி கழகம் — மதுரவாயல் தொகுதி. அனைத்து உரிமைகளும் பாதுகாக்கப்பட்டவை.",
    footerDesignedBy: "Designed and Developed by Hytide Tech"
  },
  en: {
    brandName: "TAMILAGA VETTRI KAZHAGAM",
    brandSub: "Maduravoyal Constituency",

    // Navigation
    navHome: "Home",
    navAbout: "About Us",
    navContact: "Contact",
    navComplaint: "Complaint",

    topBarText: "Voice of the People — TVK Maduravoyal Constituency",
    hotlineLabel: "Toll-Free Helpline:",

    // Home Page
    heroTagline1: "For the people, by the people",
    heroTitlePrefix: "TAMILAGA VETTRI KAZHAGAM",
    heroTitleSub: "Maduravoyal Constituency",
    heroSubtitle: "A dedicated people's movement acting as the voice and service organization for Maduravoyal.",
    heroCtaComplaint: "File a Complaint",
    heroCtaAbout: "About TVK",

    homeIntroTitle: "About Our Movement",
    homeIntroPara: "Tamilaga Vettri Kazhagam (TVK) Maduravoyal is committed to upholding social justice, public transparency, and resolving ward-level grievances for every citizen.",

    stat1Num: "50,000+",
    stat1Label: "Members",
    stat2Num: "100%",
    stat2Label: "Ward Reach",
    stat3Num: "4,800+",
    stat3Label: "Resolved Complaints",
    stat4Num: "24/7",
    stat4Label: "Public Service",

    homeQuickActionsTitle: "Quick Portals",
    homeAction1Title: "Complaint Portal",
    homeAction1Desc: "Directly submit grievances regarding roads, water supply, electricity, and civic amenities.",
    homeAction2Title: "Contact & Headquarters",
    homeAction2Desc: "Find our office location, phone numbers, and official communication channels.",

    // About Us Page
    aboutHeaderTitle: "About Us",
    aboutHeaderSub: "",
    aboutWhoTitle: "A movement working as the voice of the people and for the needs of the people",
    aboutWhoPara1: "Tamilaga Vettri Kazhagam is a people's movement working continuously to uphold the rights of the Tamil people and ensure social justice. Our policy: Every human being should live with dignity.",
    aboutWhoPara2: "",

    aboutPurposeTitle: "",
    purpose1Title: "Our Vision",
    purpose1Desc: "To create a Tamil Nadu with equal opportunities, equal justice, and equal rights for all.",
    purpose2Title: "Our Action Plan",
    purpose2Desc: "To listen to people's grievances directly and resolve them swiftly through appropriate authorities.",
    purpose3Title: "Our Approach",
    purpose3Desc: "Acting through transparency, honesty, and direct communication with the people.",

    // Contact Page
    contactHeaderTitle: "Contact Us",
    contactHeaderSub: "Official Address and Direct Communication Channels",
    contactOrgName: "TVK Maduravoyal Constituency Headquarters",
    contactPhoneTitle: "Phone Numbers",
    contactPhoneVal: "+91 44 2345 6789 / +91 98765 43210",
    contactTollFree: "Toll-Free Helpline: 1800-TVK-MAD (1800 885 623)",
    contactEmailTitle: "Email Address",
    contactEmailVal: "maduravoyal@tvk.org.in",
    contactAddressTitle: "Headquarters Address",
    contactAddressVal: "No. 42, PHC Main Road, Maduravoyal, Chennai - 600 095, Tamil Nadu, India.",
    contactHoursTitle: "Office Hours",
    contactHoursVal: "Monday - Sunday: 9:00 AM - 6:00 PM",
    contactMapTitle: "Office Location (Google Maps)",
    contactMapBtn: "Get Directions on Google Maps",
    contactSocialTitle: "Follow Us On Social Media",

    // Complaint Page
    complaintHeaderTitle: "Official Complaint Form",
    complaintHeaderSub: "Submit your complaint directly to TVK Maduravoyal",
    complaintNotice: "Note: All complaints submitted here are compiled and dispatched directly to TVK Maduravoyal executives.",

    formName: "Full Name *",
    formNamePlaceholder: "Enter your full name",
    formPhone: "Contact Phone Number *",
    formPhonePlaceholder: "+91 98765 43210",
    formEmail: "Email Address *",
    formEmailPlaceholder: "example@domain.com",
    formAddress: "Address / Ward Details *",
    formAddressPlaceholder: "Door No, Street Name, Area, Ward No",
    formSubject: "Complaint Subject *",
    formSubjectPlaceholder: "e.g. Broken streetlight on 2nd Main Rd",
    formDetails: "Complaint Description *",
    formDetailsPlaceholder: "Describe your issue in detail...",
    formSubmitBtn: "Submit Complaint",
    formMandatory: "* Fields marked with an asterisk are mandatory.",

    valRequired: "Please fill in all mandatory fields.",
    valInvalidEmail: "Please enter a valid email address.",
    valInvalidPhone: "Please enter a valid 10-digit contact number.",

    successTitle: "Complaint Successfully Submitted!",
    successMessage: "Your complaint details have been dispatched to TVK Maduravoyal official email (maduravoyal@tvk.org.in).",
    successTicketLabel: "Complaint Reference Ticket ID:",
    successNote: "Our field team will review your complaint and contact you shortly.",
    newComplaintBtn: "Submit Another Complaint",

    // Footer
    footerDesc: "Tamilaga Vettri Kazhagam Maduravoyal Constituency. A transparent people's movement for social justice.",
    footerQuickLinks: "Pages",
    footerCopyright: "TVK Maduravoyal Constituency. All rights reserved.",
    footerDesignedBy: "Designed and Developed by Hytide Tech"
  }
};
